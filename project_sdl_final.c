/**
 * project_sdl_final.c - MULTI-DINO RUN [Menu & Battle Edition]
 *
 * project_sdl_ef.c 기반 최종 개발용 버전
 *
 * 추가/수정 기능:
 *  1) 시작 메뉴: DUO MODE / SOLO MODE / RECORDS / HOW TO PLAY
 *  2) ESC 또는 메뉴 버튼을 이용한 뒤로가기 흐름
 *  3) 프로그램 실행 중에만 유지되는 솔로/듀오 기록 목록
 *  4) 듀오 모드 공격 아이템 3종: RUSH, BLIND, SPEED
 ...
 *
 * Ubuntu 컴파일:
 *   gcc project_sdl_final.c -o project_sdl_final \
...
typedef enum {
    ITEM_NONE = 0,
    ITEM_RUSH = 1,    /* 다음 장애물이 가까이 생성 */
    ITEM_BLIND = 2,   /* 상대 화면 가림 */
    ITEM_SPEED = 3    /* 상대 장애물 일시 가속 */
} ItemType;
...
static const char *item_name(ItemType item) {
    switch (item) {
        case ITEM_RUSH:  return "RUSH";
        case ITEM_BLIND: return "BLIND";
...
static void inventory_push(RoundState *s, ItemType item) {
    ...
    char msg[80];
    snprintf(msg, sizeof(msg), "ITEM GET : %s", item_name(item));
    notice(s, msg, NOTICE_DURATION);
}
...
static void apply_received_item(RoundState *s, ItemType item) {
    ...
    if (item == ITEM_RUSH) {
        ...
        notice(s, "WARNING : RUSH ATTACK", NOTICE_DURATION);
    } else if (item == ITEM_BLIND) {
        s->blind_until = now_ms() + BLIND_DURATION;
        notice(s, "WARNING : BLIND ATTACK", NOTICE_DURATION);
    } else if (item == ITEM_SPEED) {
        s->speed_until = now_ms() + SPEED_DURATION;
        notice(s, "WARNING : SPEED ATTACK", NOTICE_DURATION);
    }
}
...
static void show_help(App *app, int *quit) {
    while (!*quit) {
        ...
        draw_text(app->ren, app->font_small, "Avoid 5 obstacles to receive one random item.", 10, 15, (SDL_Color){60,60,70,255});
        draw_text(app->ren, app->font_small, "A : Use the oldest item in your inventory.", 10, 16, (SDL_Color){60,60,70,255});
        draw_text(app->ren, app->font_small, "RUSH  - Opponent's next obstacle appears closer.", 10, 18, (SDL_Color){190,90,20,255});
        draw_text(app->ren, app->font_small, "BLIND - Opponent's view is covered briefly.", 10, 19, (SDL_Color){75,75,95,255});
        draw_text(app->ren, app->font_small, "SPEED - Opponent's obstacle speeds up briefly.", 10, 20, (SDL_Color){190,50,50,255});
...
static int input_ip_screen(App *app, char *ip, size_t size, int *quit) {
    ...
    while (!*quit) {
        ...
        draw_text_center(app->ren, app->font_small,
                         "Same computer test: 127.0.0.1     ENTER : CONNECT     ESC : BACK", 440,
                         (SDL_Color){70,80,95,255});
        SDL_RenderPresent(app->ren);
        SDL_Delay(16);
    }
...
static int host_wait(App *app, NetLink *net, int *quit) {
    ...
    while (!*quit) {
        ...
        draw_text_center(app->ren, app->font_large, "WAITING FOR PLAYER", 145,
                         (SDL_Color){45,55,70,255});
        draw_text_center(app->ren, app->font, "PORT : 9000", 280,
                         (SDL_Color){60,70,90,255});
        draw_text_center(app->ren, app->font_small,
                         "Tell your friend to choose JOIN and enter this PC's IP address.", 345,
...
static void draw_inventory(App *app, const RoundState *state) {
    char label[160] = "ITEM [A] : ";
    if (state->inv_count == 0) {
        strcat(label, "EMPTY");
    } else {
        for (int i = 0; i < state->inv_count; i++) {
            strcat(label, item_name(state->inventory[i]));
            if (i + 1 < state->inv_count) strcat(label, " > ");
        }
    }
    draw_text(app->ren, app->font_small, label, 44, 3, (SDL_Color){200,135,20,255});
}
...
static void render_duo_game(App *app, const RoundState *s, const NetPacket *peer,
    ...
    if (now_ms() < s->notice_until) {
        draw_text_center(app->ren, app->font, s->notice, 100, (SDL_Color){220,60,45,255});
    }
    if (now_ms() < s->blind_until) {
        draw_filled(app->ren, 0, 2 * CELL_H, WIN_W, (DUO_MY_BASE_Y - 2) * CELL_H,
                    (SDL_Color){10,10,20,220});
        draw_text_center(app->ren, app->font_large, "BLINDED!", 175,
                         (SDL_Color){255,80,70,255});
    }
    if (now_ms() < s->speed_until) {
        draw_text(app->ren, app->font_small, "SPEED ATTACK ACTIVE", 52, 4,
                  (SDL_Color){230,60,50,255});
    }
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_KEYDOWN) {
                ...
                if (!state.dead && e.key.keysym.sym == SDLK_a && state.inv_count > 0 &&
                    ...
                    char msg[80];
                    snprintf(msg, sizeof(msg), "ATTACK SENT : %s", item_name(used));
                    notice(&state, msg, NOTICE_DURATION);
                }
            }
        }
        if (*quit) break;
